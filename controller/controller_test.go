...
func MockRegistry(lifecycleStatus string, status *api.ClusterStatus) registry.Registry {
	if status == nil {
...
func TestProcessCluster(t *testing.T) {
	versions := map[string]channel.ConfigVersion{"alpha": "<alpha-sha>"}
	...
	for _, ti := range []struct {
	...
	}{
		{
			...
			channelSource: MockChannelSource(versions, false),
		...
		},
		{
			...
			registry:      MockRegistry(statusReady, nil),
			provisioner:   &mockErrProvisioner{},
			channelSource: MockChannelSource(versions, true),
			options:       defaultOptions,
			success:       false,
		},
	} {
