...
const (
	...
	webhookIDConfigItemKey              = "webhook_id"
	maxApplyRetries                     = 10
...
func (p *clusterpyProvisioner) Provision(cluster *api.Cluster, channelConfig *channel.Config) error {
	...
	if nodePoolFeatureEnabled(cluster) {
		// TODO(tech-depth): remove the need for parsing a custom webhook ID
		if _, ok := cluster.ConfigItems[webhookIDConfigItemKey]; !ok {
			webhookID, err := parseWebhookID(cluster.ID)
			if err != nil {
				return err
			}
			cluster.ConfigItems[webhookIDConfigItemKey] = webhookID
		}
